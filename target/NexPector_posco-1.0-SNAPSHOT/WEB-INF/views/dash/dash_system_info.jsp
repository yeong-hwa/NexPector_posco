...
<head>
  ...
  <script>
    ...
    $(function() {
        $(".webwidget_scroller_tabphone").webwidget_scroller_tabphone({
            scroller_time_interval: '120000',
            scroller_window_padding: '10',
            scroller_window_width: '520',
            scroller_window_height: '32',
...
function go_page() {
	...
	location.href="dash_service_info.htm";	
/* 	if(pageTime > 0) {
		setTimeout(function() {
			location.href="dash_service_info.htm";
		}, pageTime);
...
function disp_dashSystemInfo() {
	...
	var param = {
		'firstRecordIndex'	: 0,
		'lastRecordIndex'	: 25, 
		'PHONE_GUBUN' : ipphonegubun
	};
	cmLog("ajax_system_info param: " + param);
...
function disp_nationalCallStatus(data) {
	cmLog("disp_nationalCallStatus : " + data);
	if(data === undefined) {
		data = [];
	}
...
function disp_systemErrStatus(data) {
	cmLog("disp_systemErrStatus : " + data);
...
function dispAlarmMapItem(cname, item) {
	...
	if(item.ALM_COUNT > 0) {
		...
		$(ph_id + " .nor_num").html(item.TOT_COUNT - item.ALM_COUNT);
...
function initDashAlarmBigGrid(code, sel_status) {
	...
	$("#dash_big_alarm_grid")
		.kendoGrid($.extend({}, kendoGridDefaultOpt, {
			...
			sortable	: true,
			columns		: [
				{template:'# if(N_ALM_STATUS == 0) {# <span style="color:green;font-size:20pt">●</span> #} else {# <span style="color:red;font-size:20pt">●</span> #}#', title:"<span style='font-size:20pt;'>○</span>", width:'5%', attributes:alignCenter, headerAttributes:alignCenter},
				...
				{field:'S_ADDRESS', 	title:'주소', width:'33%', attributes:alignCenter, headerAttributes:alignCenter},
				{field:'S_IP_ADDRESS', 	title:'IP_Address', width:'0px', attributes:alignCenter, headerAttributes:alignCenter}
			],
			height		: '432px'
		}));

	$("#dash_big_alarm_grid").data("kendoGrid").hideColumn("S_IP_ADDRESS");
...
function selectedServerGridRow(event) {
		...
		var param = {
			'currentPageNo'	: $('#dash_big_alarm_grid').data('kendoGrid').dataSource.page()
		};
		
		var paramStr = JSON.stringify(param);
		
		console.log("selectedServerGridRow:" + this.dataItem(this.select()).S_IP_ADDRESS);
...
function makeMidTailHtml(data) {
	...
	for(var i = 0; i < data.length; i ++) {
		...
		if((i + 1) % 4 == 0) {
			...
			sHtml += '<div class="box4_wrap fl mgr8" style="cursor:pointer" onclick="showModalMidPopupEx(\'' + ip_addr + '\', ' + (i + 1) + ')">';
		}
		
		var name	= item.S_NAME;
		
		//if(ipphonegubun == "러닝" && item.S_RUNNING != '')
		if(item.S_RUNNING != '' && name != '')
			name = item.S_NAME + " " + item.S_RUNNING;
...
function makeMidListHtml(data) {
	...
	for(var i = 0; i < data.length; i ++) {
		...
		sHtml += '<th scope="row">IP Address</th><td><input type="text" id="textipfield_' + (i + 1) + '" name="textipfield" style="height: 35px;" placeholder="" value="'+item.S_IP_ADDRESS+'"/><a class="btn_confirm" href="javascript:updatePhoneIpAddress(\'' + item.S_IP_ADDRESS + '\', \'\', ' + (i + 1) + ');" >변경</a></td>';
		...
		sHtml += '<tr>';
		sHtml += '<th scope="row">상태</th><td class="red">발생 (응답없음)</td>';
		sHtml += '</tr>';
		
		sHtml += '<tr>';
		sHtml += '<th scope="row">비고</th><td>' + bigo + '</td>';
		sHtml += '</tr>';
...
function makeSystemErrListHtml(data) {
	...
	if(data.length == 0) {
		$('#sys_err_list.conscroll').removeClass('noerror');
		$('#sys_err_list.conscroll').addClass('noerror');
	} else {
		$('#sys_err_list.conscroll').removeClass('noerror');
	}	
}
...
function showModalBigPopup(code, text, sel_status) {
	...
	if(ipphonegubun === '지점') {
		...
		$.trim($('#SEARCH_KEYWORD').val('지점'));
	} else if(ipphonegubun === '러닝') {
		$('#SEARCH_TYPE').val('S_GUBUN');
		$.trim($('#SEARCH_KEYWORD').val('러닝'));
	} 
...
function showModalMidPopupEx(ip, idx) {
	...
	for(var i = 0; i < g_arrPhoneErrList.length; i ++) {
		var item	= g_arrPhoneErrList[i];
		var ip_addr	= item.S_IP_ADDRESS;
...
function sorJijumPhoneList(sort) {
	var params 	= {
		'SORT' : sort
	};
	
	if(sort == 0) {
		$.getJSON("<c:url value='/admin/jijum.phone_alm_total_list_qry.htm'/>", param, function(data){
			console.log(data);
			disp_almTotlaList(resp.ALM_TOTAL_LIST);
		});
	} else if (sort == 1) {
		$.getJSON("<c:url value='/admin/jijum.phone_alm_total_list_qry_jijum.htm'/>", param, function(data){
			console.log(data);
			disp_almTotlaList(resp.ALM_TOTAL_LIST);
		});
	} else if (sort == 2) {
		$.getJSON("<c:url value='/admin/jijum.phone_alm_total_list_qry_running.htm'/>", param, function(data){
			console.log(data);
			disp_almTotlaList(resp.ALM_TOTAL_LIST);
		});
	}
...
function getJijumPhoneList(param, func_callback) {
 ...
 * tb_dash_alm_skip
 */
function updatePhoneIpAddress(ip, mon_id, idx) {
	var new_ipAddress	= $('#textipfield_' + idx).val();

	if(new_ipAddress == ip) {
		alert('기존 ip address와 동일 합니다.');
		return;
	}
	//alert('ip address를 입력하세요.'+ ip +", " + new_ipAddress);
	...
	var param	= {
			'S_IP_ADDRESS' :	ip
			, 'N_MON_ID' :		mon_id
			, 'NEW_S_IP_ADDRESS' :	new_ipAddress
			, 'S_USER' :		'${sessionScope.S_USER_ID}'
	}
	console.log(param);
	...
	$.getJSON("<c:url value='/admin/ins_dash_system.update_phone_ipaddress.htm'/>", param, function(data){
		console.log("update phone" + data);
		
		$.getJSON("<c:url value='/admin/ins_dash_system.del_phone_res.htm'/>", param, function(data3){
			console.log(data3);
		});		
		
		$.getJSON("<c:url value='/admin/ins_dash_system.del_alm_phone.htm'/>", param, function(data4){
			console.log(data4);
		});
		
		$.getJSON("<c:url value='/admin/ins_dash_system.del_alm_histroy_phone.htm'/>", param, function(data5){
			console.log(data5);
		});
  ...
  <div class="wrapper">
    ...
    <div class="content_wrapper">
      ...
      <div class="phone_wrap">
 		<div id="wdg_scroller_tab" class="webwidget_scroller_tabphone phone_data">
           <div class="tabContainer">
              <ul class="tabHead mgb9">
                <li class="currentBtn"><a href="javascript:goTabTotal();">전국</a></li>
                <li><a href="javascript:goTabJijum();">사업국</a></li>
                <li><a href="javascript:goTabRunning();">러닝</a></li>
              </ul>
            </div>
            <div class="tabBodyphone">  
      		</div>	
      </div>    
        <!-- title -->
...
<!--             <p class="sys_er_cicle_stit mgr20 mgb10">
            	<span style="cursor:pointer;" onclick="sorJijumPhoneList(0);">전체/</span><span style="cursor:pointer;" onclick="sorJijumPhoneList(1);"></span>지점/<span style="cursor:pointer;" onclick="sorJijumPhoneList(2);">러닝</span>
            </p>    -->           
            </ul>
          </div>
        </div>
					...
					<li class="mgl12">구분
			            <select class="form_control" name="SEARCH_TYPE" value="${param.SEARCH_TYPE}" id="SEARCH_TYPE">
							<option value="S_GUBUN">구분</option>
			          		...
			          		<option value="S_RUNNING">러닝</option>
