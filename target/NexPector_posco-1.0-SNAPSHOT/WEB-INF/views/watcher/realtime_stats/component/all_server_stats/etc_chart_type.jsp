...
<chart>
	...
	<draw>
		<text x='<%=tmp_pos-2%>' y='31' bold='true' shadow='high' color='111111' size='12'><%=all_cnt%></text>
		...
		<%
			...
			for(int i=0;i<svr_type.size();i++)
			{
				if(((HashMap)svr_type.get(i)).get("CODE").toString().equals(request.getParameter("N_TYPE_CODE")))
				{
					S_TYPE_NAME = ((HashMap)svr_type.get(i)).get("VAL").toString();
				}
			}
		%>
		...
		<text x='<%=pos-14%>' y='108' bold='true' color='000000' size='16' font='굴림'><%=S_TYPE_NAME%></text>
