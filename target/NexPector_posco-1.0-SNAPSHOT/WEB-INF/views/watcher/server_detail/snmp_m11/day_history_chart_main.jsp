...
$(document).ready(function(){
	...
	$.ajax({
		url:'<c:url value="/watcher/server_detail/accrue_chart_M11TrafficUsageDayQry.htm"/>',
        type:"post",
        data:{N_MON_ID : $('#N_MON_ID').val(), N_IF_INDEX : $('#N_IF_INDEX').val(), S_COLUMN : $('#S_COLUMN').val(), N_MONTH : $('#N_MONTH').val()},
        dataType : "text",
        success: function(res){
        	strArr = eval('('+res.replace(/\"/gi, "")+')');
			var ticks = new Array();

        	for(var i=0;i<$('#N_MONTH').val().substr(6,2);i++)
        	{
        		var tmpArr = [i+1, i+1];
        		ticks[i] = tmpArr;
        	}
...
<input type="hidden" id="N_MONTH" name="N_MONTH" value="${param.N_MONTH}" />
