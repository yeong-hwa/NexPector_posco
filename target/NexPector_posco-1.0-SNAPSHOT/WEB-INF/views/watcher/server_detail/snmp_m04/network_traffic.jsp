...
<script type="text/javascript">
		...
		var dataSource = new kendo.data.DataSource({
			transport		: {
				read		: {
					...
					url 		: cst.contextPath() + "/watcher/kendoPagination_M04IfInfoLstQry.htm",
					data 		: function(data) {
    ...
    function drawChart() {
		...
		$.getJSON('/watcher/server_detail/switchTrafficUsingHistory.htm', { 'N_MON_ID' : pMonId, 'N_INDEX' : grid.dataItem(grid.select()).N_INDEX, 'N_DAY': searchDay})
		.done(function(data) {
			console.log(data);
			$('#in_now').text('');
			$('#in_avg').text('');
...
//                max : 100,
	...
	function openDetailPopup() {
		...
		$.post(cst.contextPath() + '/watcher/go_server_detail.snmp_m04.network_traffic_popup.htm', param)
		.done(function(html) {
			$('#popup')
			.html(html)
			.dialog({
