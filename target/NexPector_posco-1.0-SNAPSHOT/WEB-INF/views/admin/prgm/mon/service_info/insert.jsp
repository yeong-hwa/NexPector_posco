...
<!-- location -->
<div class="locationBox"><div class="st_under"><h2>감시장비 서비스 등록</h2><span>Home &gt; 감시장비 서비스 관리 &gt; 감시장비 서비스 등록</span></div></div>
	...
	<div class="manager_contBox1">
		<div class="table_typ2-5">
			<table summary="" cellpadding="0" cellspacing="0">
				...
				<tr id="service_info">
					<td class="filed_A left">서비스명 <span class="btn_pack medium"><a href="#" id="append_service">추가</a></span></td>
					<td id="service_list" class="filed_B left" colspan="3"></td>
				</tr>
			</table>
			<!-- botton -->
...
//				clearFormData();
		...
		$('#append_service').on('click', function(event) {
			event.preventDefault();
			serviceCnt += 1;
			
			var innerHtml = '';
			innerHtml += ' <div class="div_service"> ';
			innerHtml += ' 	서비스 <input type="text" name="S_SERVICE_NAME" value="" class="manaint_f" style="width:200px;"/>&nbsp; ';
			innerHtml += ' 	별칭 <input type="text" name="S_ALIAS" value="" class="manaint_f" style="width:200px;"/>&nbsp; ';
			
			innerHtml += '<input type="radio" name="F_DAEMON' + serviceCnt + '" value="S" checked>SNMP &nbsp;';
			innerHtml += '<input type="radio" name="F_DAEMON' + serviceCnt + '" value="A">Agent &nbsp;';
			innerHtml += '<input type="radio" name="F_DAEMON' + serviceCnt + '" value="C">CLI &nbsp;';
			
			innerHtml += ' 	<span class="btn_pack medium"><a href="#" onclick="removeServiceField(event, this)">삭제</a></span> ';
			innerHtml += ' </div> ';

			$('#service_list').append(innerHtml);
		});
	}
	
	function removeServiceField(event, element) {
		if (event) {
			event.preventDefault ? event.preventDefault() : event.returnValue = false;
		}
		$(element).parents('.div_service').remove();
	}
	
	function fn_validation_chk() {
		var validFlag = true;
		
		$("input[name='S_SERVICE_NAME']").each(function (index){
			if(validFlag) {
				var s = $(this).val();
				if (!s) {
					validFlag = false;
	...
	function saveData() {
		var url = "<c:url value='/admin/service_info/insert.htm'/>";
	...
	function deleteInfo() {
		if (!confirm("서비스 전체를 삭제 하시겠습니까?")) {
			return;
		}
		
		var url 	= "<c:url value='/admin/service_info/delete.htm'/>",
	...
	function getJqXhrCheckDuplication(nMonId) {
		var url = "<c:url value='map_service_info.duplicationCheck.htm'/>";
	...
	function goListPage(event) {
		...
		$('#go_list_form').attr('action', cst.contextPath() + '/admin/go_prgm.mon.service_info.retrieve.htm').submit();
	}
	...
	function detailInfoDataSetting(nMonId){
		...
		$.getJSON("<c:url value='/admin/lst_service_info.select_mon_service.htm'/>", {'N_MON_ID': nMonId}, function(data){
			...
			for(var i=0; i<serviceList.length; i++) {
				...
				innerHtml += " <div class='div_service'> ";
				innerHtml += " 	서비스 <input type='text' name='S_SERVICE_NAME' value='" + serviceList[i].S_MON_NAME + "' class='manaint_f' style='width:200px;'/>&nbsp; ";
				innerHtml += " 	별칭 <input type='text' name='S_ALIAS' value='" + serviceList[i].S_ALIAS + "' class='manaint_f' style='width:200px;'/>&nbsp; ";
				
				innerHtml += "<input type='radio' name='F_DAEMON" + serviceCnt + "' value='S'";
				if (serviceList[i].F_DAEMON == 'S') {
					innerHtml += " checked";
				}
				innerHtml += ">SNMP &nbsp;";
				innerHtml += "<input type='radio' name='F_DAEMON" + serviceCnt + "' value='A'";
				if (serviceList[i].F_DAEMON == 'A') {
					innerHtml += " checked";
				}
				innerHtml += ">Agent &nbsp;";
				innerHtml += "<input type='radio' name='F_DAEMON" + serviceCnt + "' value='C'";
				if (serviceList[i].F_DAEMON == 'C') {
					innerHtml += " checked";
				}
				innerHtml += ">CLI &nbsp;";
