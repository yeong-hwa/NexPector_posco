	...
	<script>
		$(function(){
			...
			$.getJSON("<c:url value='/admin/map_${param.app_nm}.detail_info.htm'/>", param, function(data){
                ...
                if(data.N_GROUP_CODE == '11000')
                {
                  $("#N_RACK_ID option").remove();
                  $("#N_RACK_ID").append('<option value="">선택</option>');
                  cfn_makecombo_opt($("#N_RACK_ID"), "<c:url value="/admin/lst_common.cmb_rack_info.htm?N_GROUP_CODE=11000"/>");
      			  $("#rack_info").show();
...
<c:if test="${param.UPD_FLAG != 'Y'}">
	...
	function fn_save()
	{
		...
		if ($("select[name=N_GROUP_CODE]").val() == "11000" || $("select[name=N_GROUP_CODE]").val() == "20000") {
