	...
	<script>
		$(function(){
			...
			$.getJSON("<c:url value='/admin/map_${param.app_nm}.detail_info.htm'/>", param, function(data){
                ...
                if(data.N_GROUP_CODE == 100000 || (data.N_GROUP_CODE > 0 && data.N_GROUP_CODE < 26))
                {
                  $("#N_RACK_ID option").remove();
                  $("#N_RACK_ID").append('<option value="">선택</option>');
                  cfn_makecombo_opt($("#N_RACK_ID"), "<c:url value="/admin/lst_common.cmb_rack_info.htm?N_GROUP_CODE=100000"/>");
      			  $("#rack_info").show();
                  $("select[name='N_RACK_ID']").val(data.N_RACK_ID);
                }
                //else if (data.N_GROUP_CODE == '20000') {
                //  $("#N_RACK_ID option").remove();
                //  $("#N_RACK_ID").append('<option value="">선택</option>');
                //  cfn_makecombo_opt($("#N_RACK_ID"), "<c:url value="/admin/lst_common.cmb_rack_info.htm?N_GROUP_CODE=20000"/>");
      			//  $("#rack_info").show();
                //  $("select[name='N_RACK_ID']").val(data.N_RACK_ID);
                //}
                else {
...
<c:if test="${param.UPD_FLAG != 'Y'}">
	...
	function fn_save()
	{
		...
		//var grpCode = $("select[name=N_GROUP_CODE]").val();
		//if (grpCode == 100000 || (grpCode > 0 && grpCode < 26) ) { // || $("select[name=N_GROUP_CODE]").val() == "20000"

			//if($("select[name=N_DASHBOARD_MON_TYPE]").val() == "" || $("select[name=N_DASHBOARD_MON_TYPE]").val() == null){
			//	alert("장비 타입을 선택하십시오.");
			//	$('#N_DASHBOARD_MON_TYPE').focus();
			//	return;
			//}

		//	if($("select[name=N_RACK_ID]").val() == "" || $("select[name=N_RACK_ID]").val() == null){
		//		alert("랙(Rack) 이름을 선택하십시오.");
		//		$('#N_RACK_ID').focus();
		//		return;
		//	}
		//}
...
<table width="775" border="0" cellspacing="0" cellpadding="0">
  ...
  <tr>
    <td><!-- 표 start -->
              ...
              <script>
                $(function(){
                  $("select[name='N_GROUP_CODE']").change(function(){
                	var selectItem = $("select[name='N_GROUP_CODE'] option:selected").val();

                    $("#N_RACK_ID option").remove();
                    $("#N_RACK_ID").append('<option value="">선택</option>');
                    $("select[name='N_RACK_ID']").val("");

                    if(selectItem == 100000 || (selectItem > 0 && selectItem < 26))
                    {
                      cfn_makecombo_opt($("#N_RACK_ID"), "<c:url value="/admin/lst_common.cmb_rack_info.htm?N_GROUP_CODE=100000"/>");
                      $('#N_RACK_LOCATION').val("");
                      $('#N_RACK_UNIT').val("");
      ...
      <div id="rack_info" style="display: none;">
        <input type="hidden" name="N_DASHBOARD_MON_TYPE" id="N_DASHBOARD_MON_TYPE" value="0">
        <table width="100%" border="0" cellspacing="0" cellpadding="0">
          <!--
          <tr>
            <td width="20%" height="27" class="line_gray table_bg pl10 text11 b">
              장비 타입
            </td>
          ...
          <tr>
            ...
            <td class="line_gray pl10">
            	<input type="text" name="N_RACK_LOCATION" id="N_RACK_LOCATION" class="넌숫자만" style="width:150px;" value="" maxlength="2">
            </td>
          </tr>
          <tr>
            <td height="27" class="line_gray table_bg pl10 text11 b">
            ...
            <td class="line_gray pl10">
              <input type="text" name="N_RACK_UNIT" id="N_RACK_UNIT" class="넌숫자만" style="width:150px;" value="" maxlength="2">
            </td>
          </tr>
        </table>
      </div>
