...
<script type="text/javascript">
	...
	function Initialize() {
		startDay = createStartKendoDatepicker('start_date');
		...
		var dataSource = new kendo.data.DataSource({
			transport: {
				read: {
					url: cst.contextPath() + '/watcher/lst_cmb_network_interface.htm'
					, dataType: "json"
	...
	function searchNetworkTrafficChart() {
		...
		// validation 
		var temp1 = kendo.toString(startDay.value(), 'd').replace(/-/gi, "").substr(5, 2);
		var temp2 = kendo.toString(endDay.value(), 'd').replace(/-/gi, "").substr(5, 2);
		if (temp1 != temp2) {
		...
		$.getJSON('/watcher/server_detail/switchTrafficUsingDetailHistoryQry.htm', param)
		.done(function(data) {
			console.log(data);
			...
			for (var j = 0; j < statsHistory.length; j++) { // 값이 있는 것만 배열에 넣어줌
				...
				if((j+1) == statsHistory.length) {
					$('#in_now').text(comma(statsHistory[j].N_IN_OCTETS));
					$('#out_now').text(comma(statsHistory[j].N_OUT_OCTETS));
				}
			}
			...
			if (statsHistory.length > 60) {
				skipLength = (statsHistory.length / 12).toFixed(0);
			} else {
				skipLength = 5;
			}
