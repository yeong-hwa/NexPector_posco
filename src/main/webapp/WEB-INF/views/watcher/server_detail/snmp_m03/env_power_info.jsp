...
<style class="removeStyle" type="text/css">
	#power_info_listView { padding: 10px 5px; margin-bottom: -1px; }
	...
	.product img { width: 70px; height: 79px; }
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		var dataSource = new kendo.data.DataSource({
			transport		: {
				read		: {
					...
					url 		: cst.contextPath() + "/watcher/kendoPagination_M03EnvPowerInfoLstQry.htm",
					data 		: function(data) {
						return {
							'N_MON_ID' : pMonId
						};
		...
		$("#power_info_listView").kendoListView({
			dataSource : dataSource,
			template   : kendo.template($("#power_info_template").html())
		});

	});
</script>

<script type="text/x-kendo-template" id="power_info_template">
	<div class="product">
		# if (parseInt(N_STATE) === 1) {
		# <img src="<c:url value="/common/images/watcher/power_img.jpg"/>" alt="정상" /> #
		} else {
		# <img src="<c:url value="/common/images/watcher/power_warning.jpg"/>" alt="위험" /> #
		} #
		<h3>#: S_DESC #</h3>
	</div>
</script>

<div id="power_info_listView"></div>
<div id="power_info_pager" class="k-pager-wrap"></div>