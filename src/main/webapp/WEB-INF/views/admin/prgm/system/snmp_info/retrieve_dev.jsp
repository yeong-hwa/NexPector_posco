...
<%--해당 화면의 url --%>
<c:set var="app_url"><c:url value='/admin/go_prgm.system.snmp_info'/></c:set>
<c:set var="app_nm" value="snmp_info"/>

<%--해당 화면의 제목 --%>
<c:set var="app_title_name" value="SNMP 정보 관리"/>

<script>
	<%-- 데이터의 키 값(Update 및 상세 조회시 사용할 Key 값 여러개일경우 세미콜론(;) 으로 구분 --%>
	var v_key_col = "N_MON_ID;N_SNMP_MAN_CODE;S_SNMP_IP";
	...
	var v_lst_head = [
		...
		, ["SNMP IP","10%","S_SNMP_IP"]
		, ["SNMP Port","10%","N_SNMP_PORT"]
		, ["SNMP Community","30%","S_SNMP_COMMUNITY"]
		, ["SNMP Version","10%","N_SNMP_VERSION"]
	];
</script>
<body>
          ...
          <table width="100%" border="0" cellspacing="0" cellpadding="0" id="search_data">
              <tr>
                <td height="45" align="center" class="box">
                	...
                	감시장비 <cmb:combo qryname="svrComboQry2" seltagname="N_MON_ID" firstdata="전체" etc="class=\"input_search\" style=\"width:80;\""/>
                	<img src="${img1}/icon_arrow.png" align="absmiddle">
                	SNMP장비 
					...
					$("<tr/>", {
						...
						$("<form name='frm_upd' method='post' action='${app_url}.insert.htm'/>").append(
							[
								...
								, "<input type='hidden' name='app_nm' value='${app_nm}'>"
								, "<input type='hidden' name='app_url' value='${app_url}'>"
								, "<input type='hidden' name='OLD_N_MON_ID' value='" + tmp_obj.OLD_N_MON_ID + "'>"
								, "<input type='hidden' name='OLD_N_SNMP_MAN_CODE' value='" + tmp_obj.OLD_N_SNMP_MAN_CODE + "'>"
								, "<input type='hidden' name='OLD_S_SNMP_IP' value='" + tmp_obj.OLD_S_SNMP_IP + "'>"
