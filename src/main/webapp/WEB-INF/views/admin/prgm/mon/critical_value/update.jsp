...
<!-- location -->
<div class="locationBox"><div class="st_under"><h2>임계치 정보 수정</h2><span>Home &gt; 감시장비 관리 &gt; 임계치 정보 수정</span></div></div>
				...
				<tr>
					...
					<td class="bgtc1">
						...
						<span style="display:inline-block; float:right;">
							<strong>서버타입 : </strong>
							<cmb:combo qryname="common.cmbSvrTypeIncludeDump"
									   seltagname="N_TYPE_CODE"
									   selvalue="${param.N_TYPE_CODE}"
									   etc="id=\"search_svr_type\" class=\"input_search\" style=\"width:100;\" disabled=\"disabled\""/>
						</span>
					</td>
					<td class="bgtr1"></td>
				</tr>
			...
			<tr>
				...
				<td class="bgtc1">
					...
					<span style="display:inline-block; float:right;">
						...
						<select id="search_alm_type" disabled="disabled">
			...
			<tr>
				...
				<td class="bgmc1">
					<div class="table_typ2-7">
						<div id="threshold_area">
							<c:choose>
								<c:when test="${data.N_ALM_TYPE == 26001 and data.N_ALM_CODE == 0}">
									<jsp:include page="/WEB-INF/views/admin/prgm/mon/critical_value/threshold/ivr_app_threshold.jsp"/>
								</c:when>
								<c:when test="${data.N_ALM_TYPE == 16001 and data.N_ALM_CODE == 5}">
									<jsp:include page="/WEB-INF/views/admin/prgm/mon/critical_value/threshold/traffic_trunk_threshold.jsp"/>
								</c:when>
								<c:when test="${data.N_ALM_TYPE == 25001 and data.N_ALM_CODE == 2}">
									<jsp:include page="/WEB-INF/views/admin/prgm/mon/critical_value/threshold/abandoned_threshold.jsp"/>
								</c:when>
								<c:otherwise>
									<jsp:include page="/WEB-INF/views/admin/prgm/mon/critical_value/threshold/threshold.jsp"/>
								</c:otherwise>
							</c:choose>
						</div>
						<!-- botton -->
...
<script>
	...
	$(document).ready(function() {
		...
		// 임계치 설정 Combo 생성
		var almType = '${data.N_ALM_TYPE}' + ';' + '${data.N_ALM_CODE}';
		var jqXhr = $.getJSON(cst.contextPath() + '/admin/searchThresholdType.htm', {'N_TYPE_CODE':$('#search_svr_type').val()});
		console.log(almType);
		jqXhr.done(function(data) {
			...
			for (var i = 0, length = arrayData.length; i < length; i++) {
				...
				if (almType === obj.CODE) {
					$searchAlmType.append('<option selected value="' + obj.CODE + '">' + obj.VAL + '</option>');
				} else {
...
//				$("form")[0].reset();
	...
	function initServerGrid() {
		var dataSource = new kendo.data.DataSource({
			transport		: {
				read		: {
					...
					url 		: cst.contextPath() + "/admin/kendoPagination_common.svrlist.htm",
					data 		: function(data) {
						return {
							'N_TYPE_CODE' : '${param.N_TYPE_CODE}',
		...
		grid = $("#server_grid")
			.kendoGrid($.extend({}, kendoGridDefaultOpt, {
				...
				selectable	: 'multiple',
				columns		: [
					{headerTemplate: '<input type="checkbox" checked disabled="disabled" value=""/>', template: '<input type="checkbox" name="N_MON_ID" checked disabled="disabled" value="#=N_MON_ID#"/>', width:'10%', attributes:alignCenter, headerAttributes:alignCenter, sortable : false},
	...
	function critical_save() {
		...
		$('#table_threshold :input, #table_threshold [type=select]').prop('disabled', false);
		$('#server_grid table [type=checkbox]:checked').prop('disabled', false);
		...
		param += '&N_TYPE_CODE=' + $('#search_svr_type option:selected').val();
		param += '&N_MON_ID=' + $('input[name=N_MON_ID]').val();
		//-- parameters setting

		$('#table_threshold :input, #table_threshold [type=select]').not('#start_value, #end_value, #threshold, #N_USE').prop('disabled', true);
		$('#server_grid table [type=checkbox]:checked').prop('disabled', true);

		$.post(cst.contextPath() + "/admin/modifyThreshold.htm", param, function(data){
