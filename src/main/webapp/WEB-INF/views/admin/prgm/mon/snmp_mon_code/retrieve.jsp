...
<%--해당 화면의 url --%>
<c:set var="app_url"><c:url value='/admin/go_prgm.mon.snmp_mon_code'/></c:set>
<c:set var="app_nm" value="snmp_mon_code"/>

<%--해당 화면의 제목 --%>
<c:set var="app_title_name" value="SNMP 감시 상세코드 관리"/>

<script>
	<%-- 데이터의 키 값(Update 및 상세 조회시 사용할 Key 값 여러개일경우 세미콜론(;) 으로 구분 --%>
	var v_key_col = "N_SNMP_MAN_CODE;N_SNMP_MON_CODE";

	<%-- 데이터의 표현할 컬럼 및 컬럼에 대한 이름 table 테그 등 에서 사용 --%>
	var v_lst_head = [
		["SNMP장비명","15%","SNMP_MAN_NAME"]
		, ["SNMP 감시 상세코드","15%","N_SNMP_MON_CODE"]
		, ["설명","55%","S_DESC"]
		, ["기본 수집주기(분)","15%","N_TIMEM"]
	];
</script>
<body>
	<form name="frm" method="post">
          ...
          <table width="100%" border="0" cellspacing="0" cellpadding="0" id="search_data">
              <tr>
                <td height="45" align="center" class="box">
                    ...
                    SNMP장비명 <cmb:combo qryname="cmb_snmp_man_code" seltagname="N_SNMP_MAN_CODE" firstdata="전체" etc="class=\"input_search\" style=\"width:150;\""/>
                    <img src="${img1}/icon_arrow.png" align="absmiddle">
                	감시 상세 설명 <input type="text" name="S_DESC" value="${param.S_DESC}" class="input_search" id="textfield1" style="width:150px;height:20px;" onkeydown="if (kendo.keys.ENTER === event.keyCode){ javascript:fn_retrieve('1');return false;}">
                	</span>
        ...
        <tr>
          <td height="30" align="right" valign="top">
            ...
            &nbsp;
            <!-- <input style="width:80px; height:23px;" onClick="fn_excel()" type="button" value="Excel다운"/> -->
          </td>
        </tr>
        <tr>
          <td>
		...
		$.post("<c:url value='/page_${app_nm}.select_list.htm'/>", param, function(str){
			...
			<!-- 검색된 데이타가 없을때 시작 -->
				...
				jQuery.each(array, function(idx, value){
					var tmp_obj = this;
					list_body_str += "<td align='center' colspan='6'>&nbsp; 검색된 데이타가 없음 &nbsp;</td>";
				})
