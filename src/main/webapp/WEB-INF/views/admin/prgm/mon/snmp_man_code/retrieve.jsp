...
<%--해당 화면의 url --%>
<c:set var="app_url"><c:url value='/admin/go_prgm.mon.snmp_man_code'/></c:set>
<c:set var="app_nm" value="snmp_man_code"/>

<%--해당 화면의 제목 --%>
<c:set var="app_title_name" value="SNMP 장비코드 관리"/>

<script>
	<%-- 데이터의 키 값(Update 및 상세 조회시 사용할 Key 값 여러개일경우 세미콜론(;) 으로 구분 --%>
	var v_key_col = "N_SNMP_MAN_CODE";

	<%-- 데이터의 표현할 컬럼 및 컬럼에 대한 이름 table 테그 등 에서 사용 --%>
	var v_lst_head = [
		["SNMP장비코드","35%","N_SNMP_MAN_CODE"]
		, ["SNMP장비명","65%","S_DESC"]
	];
</script>
<body>
          ...
          <table width="100%" border="0" cellspacing="0" cellpadding="0" id="search_data">
              <tr>
                <td height="45" align="center" class="box">
                	...
                	SNMP장비명 <input type="text" name="S_DESC" value="${param.S_DESC}" class="input_search" id="textfield1" style="width:90px;height:20px;">
                	</span>
